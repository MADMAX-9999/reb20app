    ...
    if os.path.exists(PRESET_FOLDER):
        for filename in os.listdir(PRESET_FOLDER):
            if filename.endswith(".json"):
                ...
                with open(os.path.join(PRESET_FOLDER, filename), "r") as f:
                    st.session_state.saved_presets[preset_name] = json.load(f)
...
with st.sidebar.expander("💾 Presety", expanded=False):
    ...
    if st.button("Zapisz preset"):
        ...
        try:
            ...
            pass  # Ignoruj błędy zapisu na Streamlit Cloud
